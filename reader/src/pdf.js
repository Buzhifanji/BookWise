...
// const pdfjsWorker = await import('./vendor/pdfjs/pdf.worker.js')
...
const renderViewport = (scaleSize, page) => {
  const naturalPdfSize = page.getViewport({ scale: scaleSize })
...
const renderDocument = async (page, index, scaleSize) => {
  const { viewport } = renderViewport(scaleSize, page)
  ...
  const doc = `
    <img class="text-layer-img" src="${canvas.toDataURL()}">
    ${container.outerHTML}
    ${div.outerHTML}
  `
...
export const makePDF = async (file) => {
  ...
  const cache = new Map()
  book.init = async (scale = 1) => {
    const { viewport, scale: newScale } = renderViewport(scale, await pdf.getPage(1))
    book.scale = newScale
    book.viewport = { width: viewport.width, height: viewport.height }
  }
  book.sections = Array.from({ length: pdf.numPages }).map((_, i) => ({
    ...
    createDocument: async () => {
      ...
      const res = await renderDocument(await pdf.getPage(i + 1), i, book.scale)
      cache.set(i, res)
      return res
    },
    size: 1000
  }))
  book.destroy = () => cache.clear()
