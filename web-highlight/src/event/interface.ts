...
export interface EventHandlerMap<T> {
  ...
  [EventTypeEnum.SOURCE]: (data: { source: DomSource, isPainted: boolean, removeIds: string[], range: Range }, h: T) => void
  [EventTypeEnum.CLICK]: (data: { id?: string, target: HTMLElement, source?: DomSource }, h: T, e: MouseOrTouchEvent) => void;
  ...
  [EventTypeEnum.CREATE]: (data: { sources: DomSource; type: CreateFrom, removeIds?: string[] }, h: T) => void;
